...
START_TEST(s21_pow_test30) {
  double x[14] = {1.25,         -77.73, 8,  7,  -32,      -33,
                  2.2,          -5.6,   -9, 27, -S21_NAN, S21_INF_POS,
                  -S21_INF_POS, S21_NAN};
  double zero = 0;
  double pow_rez = 0;
  for (int i = 0; i < 13; i++) {
...
START_TEST(s21_pow_test31) {
  double x[14] = {1.25,         -77.73, 8,  7,  -53,      72,
                  2.2,          -5.6,   -9, 53, -S21_NAN, S21_INF_POS,
                  -S21_INF_POS, S21_NAN};
  double zero = -0;
  double pow_rez = 0;
...
START_TEST(s21_pow_test32) {
  ...
  double y_digit[7] = {7.77,         -7.77,   S21_INF_POS,
                       -S21_INF_POS, DBL_MAX, DBL_MIN};
  int k = 0;
  for (int i = 0; i < 2; i++) {
    k = 0;
    for (int j = 0; j < 6; j++) {
...
START_TEST(s21_pow_test33) {
  double one = -1;
  double x[7] = {1, 3.23, -3.23, -S21_NAN, S21_INF_POS, -S21_INF_POS, S21_NAN};
  double pow_rez = 0;
  double pow_rez_1 = 0;
  for (int i = 0; i < 7; i++) {
    pow_rez = pow(one, x[i]);
...
START_TEST(s21_pow_test37) {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      else if (s21_fabs(pow_rez) == s21_fabs(S21_INF_POS))
